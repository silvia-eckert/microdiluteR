---
title: "Tidy photometer data with R package `microdiluteR`"
...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 4
vignette: >
  %\VignetteIndexEntry{Tidy photometer data with R package `microdiluteR`}
...
During my work as a postdoc in multiple labs, I experienced that old photometer devices are still in use even if corresponding proprietary software (e.g. being dependent on an outdated OS) is no longer available. This is a good sign from an ecological point of view (i.e. avoidance of electronic waste) but it’s more often rather a question of the laboratory’s financial resources. However, absorption values (e.g. in a 8 x 12 table format in plain text files for 96-well plates) returned from these devices are usually analyzed via a custom Excel sheet that someone created long ago and everyone relies on it. This might work since each photometer type provides their own file structure but Excel is a proprietary software itself, i.e. not easily available for everyone. Also, the steps are tedious for most cases because only some of the analyses can be carried out in it and only limited experimental designs are supported. In contrast, R is an established open-source statistical tool, so why not also use it as a tool to tidy photometer plate data, to flexibly adding experimental designs as metadata and even conduct at least some simple visualizations and statistical analyses, all in one go?

![<b>A [96-well photometer plate](https://en.m.wikipedia.org/wiki/File:96-Well_plate.svg) is often used for broth microdilution assays.</b>](https://upload.wikimedia.org/wikipedia/commons/0/07/96-Well_plate.svg)
